...
listWebData<-function(urlTble, datasetName, dfile){
  ...
  if (typeConn == "ftp://"){
    if (isTRUE(dfile == "all")){
      ...
      if (is.na(password)) { file.list <- getURL(url2data, ftp.use.epsv = FALSE,
                                                 dirlistonly = TRUE)
      ...
      if (is.na(password)) {file.list<-paste(url2data, dfile, sep="")
      }else{
...
urls<- data.table(dataset = c("AHCCD_daily",
                  ...
                  password= c(NA,
                              ...
                              NA,
                              NA),
                  files= list(c("Adj_Daily_Rain_v2016.zip","Adj_Daily_Snow_v2016.zip","Adj_Daily_TotalP_v2016.zip",
